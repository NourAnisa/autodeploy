...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Jalankan sebagai root: sudo bash server/install-local.sh"
  exit 1
fi

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
vm_ip="${1:-$(ip -4 route get 1.1.1.1 2>/dev/null | awk '{for (i=1; i<=NF; i++) if ($i=="src") {print $(i+1); exit}}')}"

if [[ ! "$vm_ip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
  echo "IP VM tidak ditemukan. Jalankan: sudo bash server/install-local.sh 192.168.1.50"
  exit 2
fi
...
DEBIAN_FRONTEND=noninteractive apt-get install -y \
  ...
  docker.io git gnupg openssl php-cli php-fpm php-zip unzip zip
...
cat > /etc/autodeploy/config <<CONFIG
...
echo
echo "============================================================"
echo " AutoDeploy lokal berhasil dipasang"
echo "============================================================"
echo "IP VM      : $vm_ip"
echo "Deploy URL : http://deploy.$base_domain/deploy.php"
echo "Token      : $token"
echo
echo "Uji aplikasi contoh:"
echo "  bash examples/deploy-local.sh demo-app examples/php-native"
echo
echo "Buka dari laptop:"
echo "  http://demo-app.$base_domain"
echo "============================================================"
